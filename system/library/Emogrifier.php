...
class Emogrifier
{
    ...
    public static function normalizedCallback(array $m) {
    ...
    {
        if ($this->html === '') {
            throw new exception('Please set some HTML first before calling emogrify.', 1390393096);
        }
    ...
    private function clearCache($key)
    {
        ...
        if (!in_array($key, $allowedCacheKeys, true)) {
            throw new exception('Invalid cache key: ' . $key, 1391822035);
        }
    ...
    private function splitCssAndMediaCallback($matches) {
      $this->splitMedia .= $matches[0];
    }
    ...
    private function splitCssAndMediaQuery($css)
    {
        ...
        return array('css' => $css, 'media' => $this->splitMedia);
    }
     ...
     * @return string
     */
    private static function strtolowerCallback(array $matches) {
