...
$data['custom_imgtitle'] = $product_info['custom_imgtitle'];
			...
			if (isset($manufacturer_info['image']) && !empty($manufacturer_info['image'])) {
				if ($this->request->server['HTTPS']) {
					$data['manufacturer_logo'] = $this->config->get('config_ssl') . 'image/' . $manufacturer_info['image'];
				} else {
					$data['manufacturer_logo'] = $this->config->get('config_url') . 'image/' . $manufacturer_info['image'];
				}
			} else {
...
$data['description'] = ($product_info['custom_h2'] != '')?'<h2>'.$product_info['custom_h2'].'</h2>'.$data['description']:$data['description'];
			...
			$data['attribute_groups'] = $this->model_catalog_product->getProductAttributes($this->request->get['product_id']);
				...
				'img-width'       => $this->config->get('config_image_related_width'),
				'img-height'       => $this->config->get('config_image_related_height'),
            ...
            // get contents of the Whats this? modal
            $this->language->load('information/information');
            $this->load->model('catalog/information');
            $information_id = 13;
            $information_info = $this->model_catalog_information->getInformation($information_id);
            $data['whats_this_modal_title'] = $information_info['title'];
            $data['whats_this_modal_body'] = html_entity_decode($information_info['description'], ENT_QUOTES, 'UTF-8');
