...
$data['text_wishlist2'] = (isset($this->session->data['wishlist']) ? count($this->session->data['wishlist']) : 0);
    ...
    'category_id' => $category['category_id'],
                ...
                $result = $this->db->query("SELECT code FROM `" . DB_PREFIX . "extension` WHERE `code` = 'top_header_links'");

                if($result->num_rows) {

                $this->load->model('tool/top_header_links');
                $data['headermenu'] = $this->model_tool_top_header_links->getHeadermenu();

                foreach ($data['headermenu'] as $link) {
                    $children_data = array();

                    if(isset($link[$link['main_group_id']])){

                        foreach ($link[$link['main_group_id']] as $sublink) {

                        $children_data[] = array(
                            'name'  => $sublink['sub_link_name'],
                            'href'  => $sublink['sub_link']
                            );
                        }
                    }

                    $data['categories'][] = array(
                        'name'     => $link['main_link_names'],
                        'children' => $children_data,
                        'column'   => $link['columns'] ? $link['columns'] : 1,
                ...
                $data['categories_html'] = $this->model_design_topmenu->getMenu($data['categories']);
