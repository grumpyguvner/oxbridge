...
class ModelCheckoutOrder extends Model {
	...
	public function addOrder($data) {
		...
		$order_id = $this->db->getLastId();
				...
				$this->db->query("INSERT INTO `" . DB_PREFIX . "oe_order` SET `order_id` = '" . (int)$order_id . "'");
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				$this->db->query("INSERT INTO `" . DB_PREFIX . "oe_order_product` SET `oe_order_id` = '" . (int)$oe_order_id . "', `order_product_id` = '" . (int)$order_product_id . "', `order_id` = '" . (int)$order_id . "', `product_id` = '" . (int)$product['product_id'] . "', `notax` = '" . (int)$product['notax'] . "'");
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info) {
			...
			//Add Coupon History - ABANDONED CARTS
			...
			if (isset($abandonedCartsSettings['abandonedcarts']['Enabled']) && $abandonedCartsSettings['abandonedcarts']['Enabled']=='yes') {
				...
				foreach ($coupon_query->rows as $order_total) {
					...
					if (method_exists($this->{'model_total_coupon'}, 'confirm')) {  
