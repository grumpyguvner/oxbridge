...
<div id="content">
	<div class="page-header">
		<div class="container-fluid">
			<div class="pull-right"><a onclick="cancel();" class="btn btn-default"><i class="fa fa-reply"></i> <?php echo $button_cancel; ?></a></div>
								...
								<div class="tab-pane active" id="tab-product">
									<fieldset>
										...
										<div class="form-group">
											...
											<div class="col-sm-10">
												<input type="text" name="product" value="" id="input-product" placeholder="<?php echo $help_product_autocomplete; ?>" class="form-control" />
										...
										<div class="form-group">
											<label class="col-sm-2 control-label" for="input-custom-price"><?php echo $entry_custom_price; ?></label>
											<div class="col-sm-10">
												<div class="checkbox">
													<label>
														<input type="checkbox" name="custom_price" id="input-custom_price" value="1" />
													</label>
												</div>
											</div>
										</div>
										<div class="form-group">
											...
											<div class="col-sm-10">
												<input type="text" name="price" value="" id="input-price" class="form-control" disabled="disabled" />
											</div>
										</div>
										<div class="form-group">
											<label class="col-sm-2 control-label" for="input-notax"><?php echo $entry_notax; ?></label>
											<div class="col-sm-10">
												<div class="checkbox">
													<label>
														<input type="checkbox" name="notax" id="input-notax" value="1" />
													</label>
												</div>
											</div>
										</div>
						...
						<div class="tab-pane" id="tab-total">
							...
							<div class="table-responsive">
								<table class="table table-bordered">
									...
									<tbody id="total">
										<tr>
											<td class="text-center" colspan="<?php echo $t_columns; ?>"><?php echo $text_no_results; ?></td>
										</tr>
									</tbody>
								</table>
							</div>
							<fieldset>
								...
								<div class="form-group" id="add-customer">
									<label class="col-sm-2 control-label" for="input-add-customer"><?php echo $entry_add_customer; ?></label>
									<div class="col-sm-4">
										<div>
											<div class="checkbox">
												<label>
													<input type="checkbox" id="input-add-customer" name="save_customer" value="1" />
												</label>
											</div>
										</div>
									</div>
									<label class="col-sm-2 control-label" for="input-notify-customer"><?php echo $text_notify_customer; ?></label>
									<div class="col-sm-4">
										<div>
											<div class="checkbox">
												<label>
													<input type="checkbox" id="input-notify-customer" name="notify_customer" value="1" />
												</label>
											</div>
										</div>
									</div>
	...
	<script type="text/javascript"><!--
		...
		$('#tab-cart').delegate('.btn-default', 'click', function() {
			...
			$.ajax({
				url: 'index.php?route=sale/order/api&token=<?php echo $token; ?>&api=api/cart/edit',
				type: 'post',
				dataType: 'json',
				data: post_data,
				...
				success: function(json) {
					...
					if (json['products']) {
						for (i = 0; i < json['products'].length; i++) {
							product = json['products'][i];
							html += '<tr>';
							html += '  <td class="text-left">' + product['name'] + ' ' + (!product['stock'] ? '<span class="text-danger">***</span>' : '') + '</td>';
							<?php if ($product_column_option) { ?>
								html += '  <td class="text-left">';
								if (product['option']) {
									for (j = 0; j < product['option'].length; j++) {
					...
					if (json['totals']) {
						p_columns2 = p_columns - 2;
						for (i in json['totals']) {
							total = json['totals'][i];
							html += '<tr>';
							html += '  <td class="text-right" colspan="' + p_columns2 + '">' + total['title'] + ':</td>';
						...
						$.ajax({
							...
							success: function(json) {
								if (json['error']) {
									...
									$.ajax({
										url: 'index.php?route=sale/order/api&token=<?php echo $token; ?>&api=api/order_entry/setPaymentMethod&store_id=' + $('select[name=\'store_id\'] option:selected').val(),
										type: 'post',
										dataType: 'json',
										data: 'payment_method=' + encodeURIComponent($('select[name=\'payment_method\']').val()),
										success: function(json) {
											$('select[name=\'payment_method\']').val(json['payment_code']);
											$('#button-refresh').trigger('click');
										},
										error: function(xhr, ajaxOptions, thrownError) {
											alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
						...
						$.ajax({
							...
							success: function(json) {
								if (json['error']) {
									...
									$.ajax({
										url: 'index.php?route=sale/order/api&token=<?php echo $token; ?>&api=api/order_entry/setShippingMethod&store_id=' + $('select[name=\'store_id\'] option:selected').val(),
										type: 'post',
										dataType: 'json',
										data: 'shipping_method=' + encodeURIComponent($('select[name=\'shipping_method\']').val()),
										success: function(json) {
											$('select[name=\'shipping_method\']').val(json['shipping_code']);
											$('#button-refresh').trigger('click');
										},
										error: function(xhr, ajaxOptions, thrownError) {
											alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
		...
		$('#button-save').on('click', function() {
			...
			$.ajax({
				url: url,
				type: 'post',
				data: $('#tab-total select[name=\'order_status_id\'], #tab-total input[type=\'checkbox\']:checked, #tab-total select, #tab-total textarea[name=\'comment\'], #tab-total input[name=\'affiliate_id\']'),
				dataType: 'json',
				beforeSend: function() {
