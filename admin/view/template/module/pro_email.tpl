<?php function mailPreviewPanel($_language, $order_statuses, $mail_custom, $store_id, $token, $mijourl) { ?>
<div class="mail-preview panel panel-default">
  ...
  <div class="preview-content">
    <iframe src="index.php?<?php echo $mijourl; ?>route=module/pro_email/preview&store_id=<?php echo $store_id; ?>&token=<?php echo $token; ?>"></iframe>
  </div>
</div>
<?php } ?>
  ...
  <div id="stores" class="form-inline" <?php if ($OC_V2 && 0) echo 'class="v2"'; ?>>
      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
       ...
       <div class="col-md-6">
        ...
        <div class="col-md-6">
          <?php echo mailPreviewPanel($_language, $order_statuses, $mail_custom, $store_id, $token, $mijourl); ?>
        </div>
      </div>
              ...
              <tr>
                <td class="form-horizontal">
                  ...
                  <div class="form-group form-inline">
                    <label for="color_btn" class="col-sm-2 control-label"><span data-toggle="tooltip" title="<?php echo $_language->get('entry_layout_width_i'); ?>"><?php echo $_language->get('entry_layout_width'); ?></span></label>
                    <div class="col-sm-4">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-4">
                      <input name="proemail_color[btn]" data-target="a.button" data-property="background-color" type="text" id="color_btn" class="form-control minicolors" value="<?php echo isset($proemail_color['btn']) ? $proemail_color['btn'] : ''; ?>" />
                    </div>
                    <label for="color_btn_text" class="col-sm-2 control-label"><?php echo $_language->get('entry_color_btn_text'); ?></label>
                    <div class="col-sm-4">
                      <input name="proemail_color[btn_text]" data-target="a.button" data-property="color" type="text" id="color_btn_text" class="form-control minicolors" value="<?php echo isset($proemail_color['btn_text']) ? $proemail_color['btn_text'] : ''; ?>" />
                    </div>
                  </div>
              ...
              <tr>
                <td class="form-horizontal">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-10">
                      <input name="proemail_color[bg_body]" data-target=".container" data-property="background-color" type="text" id="color_bg_body" class="form-control minicolors" value="<?php echo isset($proemail_color['bg_body']) ? $proemail_color['bg_body'] : ''; ?>" />
                    </div>
                  </div>
                  <div class="form-group">
                    <label class="col-sm-2 control-label"><?php echo $_language->get('entry_background_image'); ?></label>
        ...
        <div class="tab-content clearfix">
          ...
          <div class="tab-pane" id="tab-content-4">
            ...
            <ul class="nav nav-pills nav-stacked col-md-2 menu-custom">
              ...
              <li <?php if($f) echo 'class="active"'; $f=0; ?>><a href="#tab-type-<?php echo $row; ?>" data-type="<?php echo $key; ?>" data-toggle="pill"><?php echo $item['name']; ?></a></li>
              <?php $row++; } ?>
            </ul>
            <div class="tab-content col-md-10">
    ...
    <div class="tab-pane" id="tab-3">
			<table class="form">
				<tr>
				  <td><?php echo $_language->get('entry_bg_mode'); ?></td>
				  <td><input class="switch" type="checkbox" name="proemail_bg_mode" id="proemail_bg_mode" value="1" <?php /*if($proemail_bg_mode) echo 'checked="checked"';*/ ?>/></td>
				</tr>
			</table>
		</div>
			...
			<table class="form about">
				...
				<tr>
					...
					<td>
						...
						<b>Module page :</b> <a target="new" href="http://www.opencart.com/index.php?route=extension/extension/info&extension_id=21842">Pro Email Template</a><br/>
...
jQuery('select[name=store]').change(function(){
	document.location = 'index.php?<?php echo $mijourl; ?>route=module/pro_email&token=<?php echo $token; ?>&store_id='+jQuery(this).val();
});
...
function reloadPreview() {
  ...
  <?php } ?>
	jQuery.ajax({
		url: 'index.php?<?php echo $mijourl; ?>route=module/pro_email/previewParams&token=<?php echo $token; ?>',
    type:'POST',
      ...
      //jQuery(target).closest('.mail-preview').find('iframe').attr('src', 'index.php?route=module/pro_email/preview&token=<?php echo $token; ?>');
      jQuery('form > div.tab-content > div.active .mail-preview iframe').attr('src', 'index.php?<?php echo $mijourl; ?>route=module/pro_email/preview&type='+type+'&store_id=<?php echo $store_id; ?>&token=<?php echo $token; ?>');
...
jQuery('#restoreContent').on('click', function (e) {
  ...
  if(confirm('<?php echo $_language->get('text_confirm_restore_content'); ?>')) {
    document.location = 'index.php?<?php echo $mijourl; ?>route=module/pro_email/restore_content&token=<?php echo $token; ?>&store_id='+jQuery(this).val();
  }
});
...
jQuery('#save_scheme').on('click', function (e) {
  ...
  jQuery.ajax({
		url: 'index.php?<?php echo $mijourl; ?>route=module/pro_email/saveColorScheme&token=<?php echo $token; ?>',
    type:'POST',
...
jQuery('body').on('change', '.preview-type', function() {
  reloadPreview();
});
...
var editor = jQuery(this).attr('id');
  <?php if ($OC_V2) { ?>
    ...
    <?php } else { ?>
      ...
      if (editor.search('proemail_type-common') === 0) {
        ...
        jQuery('#'+editor).summernote({height: 200});
      }
    <?php } ?>
  ...
  <?php } else { ?>
    CKEDITOR.replace(editor, {
      height:'300px',
      filebrowserBrowseUrl: 'index.php?<?php echo $mijourl; ?>route=common/filemanager&token=<?php echo $token; ?>',
      filebrowserImageBrowseUrl: 'index.php?<?php echo $mijourl; ?>route=common/filemanager&token=<?php echo $token; ?>',
      filebrowserFlashBrowseUrl: 'index.php?<?php echo $mijourl; ?>route=common/filemanager&token=<?php echo $token; ?>',
      filebrowserUploadUrl: 'index.php?<?php echo $mijourl; ?>route=common/filemanager&token=<?php echo $token; ?>',
      filebrowserImageUploadUrl: 'index.php?<?php echo $mijourl; ?>route=common/filemanager&token=<?php echo $token; ?>',
      filebrowserFlashUploadUrl: 'index.php?<?php echo $mijourl; ?>route=common/filemanager&token=<?php echo $token; ?>'
    });
  <?php } ?>
});
//--></script> 
...
jQuery('body').on('click', '.info-btn', function() {
  ...
  jQuery('#modal-info .modalContent').load('index.php?<?php echo $mijourl; ?>route=module/pro_email/modal_info&token=<?php echo $token; ?>', {'info': jQuery(this).attr('data-info')});
});
...
function image_upload(field, thumb) {
	...
	jQuery('#content').prepend('<div id="dialog" style="padding: 3px 0px 0px 0px;"><iframe src="index.php?<?php echo $mijourl; ?>route=common/filemanager&token=<?php echo $token; ?>&field=' + encodeURIComponent(field) + '" style="padding:0; margin: 0; display: block; width: 100%; height: 100%;" frameborder="no" scrolling="auto"></iframe></div>');
	...
	jQuery('#dialog').dialog({
		...
		close: function (event, ui) {
			if (jQuery('#' + field).attr('value')) {
				jQuery.ajax({
					url: 'index.php?<?php echo $mijourl; ?>route=common/filemanager/image&token=<?php echo $token; ?>&image=' + encodeURIComponent(jQuery('#' + field).val()),
					dataType: 'text',
					success: function(data) {
  ...
  var file_input = $(this).parent().parent().find('input.fileinput');
	...
	timer = setInterval(function() {
		if ($('#form-upload input[name=\'file\']').val() != '') {
			...
			$.ajax({
				url: 'index.php?route=module/pro_email/fileupload&token=<?php echo $token; ?>',
				type: 'post',		
				dataType: 'json',
