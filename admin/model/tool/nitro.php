...
class ModelToolNitro extends Model {
    ...
    public function tryChmod() {
        ...
        $nitro_folders = array(
            ...
            $site_root . DS . 'system' . DS . 'nitro' . DS . 'data',
            $site_root . DS . 'system' . DS . 'nitro' . DS . 'temp',
            $site_root . DS . 'system' . DS . 'nitro' . DS . 'temp' . DS . 'js',
            $site_root . DS . 'system' . DS . 'nitro' . DS . 'cache' . DS . 'pagecache',
            $site_root . DS . 'system' . DS . 'nitro' . DS . 'cache' . DS . 'headers',
            $site_root . DS . 'system' . DS . 'nitro' . DS . 'cache' . DS . 'dbcache'
        );
    ...
    public function getServerInfo($permission) {
        ...
        if (!$permission) $result['path_assets_css'] = $text_no_permission;
            ...
            if (file_exists(DIR_SYSTEM . '../assets/css') && is_writable(DIR_SYSTEM . '../assets/css')) $result['path_assets_css'] = '<span class="info-success"><strong>Writable</strong></span>';
            else if (!file_exists(DIR_SYSTEM . '../assets/css')) $result['path_assets_css'] = '<span><strong>Does not exist.</strong></span>';
        ...
        if (!$permission) $result['path_assets_js'] = $text_no_permission;
            ...
            if (file_exists(DIR_SYSTEM . '../assets/js') && is_writable(DIR_SYSTEM . '../assets/js')) $result['path_assets_js'] = '<span class="info-success"><strong>Writable</strong></span>';
            else if (!file_exists(DIR_SYSTEM . '../assets/js')) $result['path_assets_js'] = '<span><strong>Does not exist.</strong></span>';
        ...
        if (!$permission) $result['path_system_nitro_temp'] = $text_no_permission;
            ...
            if (file_exists(DIR_SYSTEM . 'nitro/temp') && is_writable(DIR_SYSTEM . 'nitro/temp')) $result['path_system_nitro_temp'] = '<span class="info-success"><strong>Writable</strong></span>';
            else if (!file_exists(DIR_SYSTEM . 'nitro/temp')) $result['path_system_nitro_temp'] = '<span><strong>Does not exist.</strong></span>';
    ...
    public function cron_command($config, $full_command = true) {
        ...
        $cron_command = array(
            ...
            'command' => $cmd . ' '. NITRO_CORE_FOLDER . 'cron.php >/dev/null 2>&1'
        );
