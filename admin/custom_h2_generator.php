...
if ((!isset($_GET['token'])) || ($_GET['token'] != $session->data['token'])) 	{
		...
		if (isset($data)) {$parameters = $data['seopack_parameters'];}
			else {$parameters['ch2s'] = '';}
			
		if (!isset($parameters['ch2s'])) {$parameters['ch2s'] = '';}
		...
		foreach ($query->rows as $product) {
			echo 'Generating Custom h2s for <b>'.$product['pname'].' (from '.$product['cname'].')</b>: ';
			...
			$ch2s = str_replace($bef, $aft,  $parameters['ch2s']);
			$db->query("update " . DB_PREFIX . "product_description set custom_h2 = '". htmlspecialchars($ch2s) ."' where product_id = ".$product['product_id']." and language_id = ".$product['language_id'].";");			
			
			echo " - ".( (!$parameters['ch2s']) ?" No ":"<span style=\"color:red;\">$ch2s</span> ")." custom h2 was set;<br>";
			}
	}
